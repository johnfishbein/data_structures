...
mergeSort:
	...
	push r14		;middle index
	
	cmp rsi, rdx		
	jge done		;if left index >= right index jump to return
	
	
	mov r12, rsi		;store left indec in r12
	mov r13, rdx		;store right index in r13
	
	mov r14, r13		;middle index = (left index + right index)/2
	add r14, r12
	shr r14, 1
	

	mov rsi, r12		;load left index into second parameter
	mov rdx, r14		;load middle index into third parameter
	call mergeSort		;recurse

	mov rsi, r14		;load middle index +1 into second parameter
	inc rsi			
	mov rdx, r13		;load right index into third parameter
	call mergeSort		;recurse

	push r10		;save r10 and r11 so that merge works
	push r11

	mov rsi, r12		;load left index into second parameter	
	mov rdx, r14		;load middle index into third parameter
	mov rcx, r13		;load right index into fourth parameter
	...
	pop r14			;restore r14,r13,r12 after each call to func
	pop r13
	pop r12
	ret
